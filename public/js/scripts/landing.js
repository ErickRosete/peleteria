...
// https://stackoverflow.com/questions/12991351/css-force-image-resize-and-keep-aspect-ratio
  function updateSlide(){
    console.log("===active")
    console.log(document.querySelector("div.active").id)
    if(document.querySelector("div.active").id=="special")
    {
      console.log("====llegamos al caso especial")
      $('#textContainer').hide()
      if ( $(window).width() > 500 ) {
        $(".carousel_landing").css("max-width","70%")
      }
      $(".carousel_landing").removeClass("col-md-7")
      // console.log(document.querySelectorAll("div.active")[0].children[2].children)
    }
    else{
      let small=$(".carousel_landing").hasClass("col-md-7")
      $(".carousel_landing").css("max-width","100%")
      console.log(small)
      if(!small){
        let small=$(".carousel_landing").addClass("col-md-7")
      }
      $('#textContainer').show()
